        ...
        async dispatch => {
            try {
                ...
                const [shortsResponse, normalResponse] = await Promise.all([
                    axios.get(`${process.env.REACT_APP_YOUTUBE_BASE_URL}search?part=snippet&q=${search}+shorts&type=video&maxResults=6&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`),
                    axios.get(`${process.env.REACT_APP_YOUTUBE_BASE_URL}search?part=snippet&q=${search}&type=video&maxResults=6&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
                ]);
